...
Cập nhật: 2026-07-29 · Trạng thái: ACTIVE · Thay thế cách tiếp cận cũ trong `docs/` (đã defer, xem `tracking/DEFERRED.md`).
...
- Đầu ra: lời khuyên **theo thời gian** — khi nào nên chạy, khi nào nên nghỉ/sạc — và (mở 2026-07-21, quyết định Cường sau verify) **gợi ý khu vực đứng chờ theo heatmap demand mock CÓ ĐIỀU KIỆN**: chỉ giữa các cuốc/trước ca, capacity-aware chống dồn cung, kèm cảnh báo tỷ lệ nhận cuốc, nhãn mock + bất định, không hứa thu nhập; căn cứ verify: Xanh không có heatmap tài xế và không ràng buộc khu vực realtime → tính năng bổ sung, không chồng đè tối ưu sẵn có (chi tiết + 5 điều kiện: `research/simulation/action-space.md` §Phạm vi advisor). Vẫn KHÔNG can thiệp dispatch/matching, KHÔNG khuyên nhận/từ chối/hủy đơn cụ thể. **⚠ Cập nhật 2026-07-28:** Cường duyệt bật kênh vị trí **trong SIM** (`positioning_overrides: wait_only` + capacity ledger, xem `configs/pilot_dongda.yaml`) để nghiên cứu hiệu quả/rủi ro dồn cung; ranh giới **product vẫn giữ D-004** (không reposition tài xế thật ngoài phạm vi giả lập).
- Cập nhật voucher ngay trong ca.
- **Defer:** nhận thêm state pin xe. **⚠ Cập nhật 2026-07-29:** trong khu Mô phỏng/Track UI, UI hiện đã hiển thị SOC **MOCK có nhãn `MÔ PHỎNG`** đi kèm dữ liệu (Q-06 chốt phương án (b), UPDATE-080: nhãn theo qua trường `soc_source`, badge + 2 card giải thích nguồn) — đây là nhãn hoá số mock đã tồn tại, KHÔNG phải "state pin xe thật" đưa vào sản phẩm; defer state pin thật (D-002) vẫn giữ nguyên.
...
**⚠ Đính chính 2026-07-29:** thứ tự M0→M4 liệt kê dưới đây là ROADMAP đã duyệt, KHÔNG phải trình tự đã thực chạy. Chương trình **thực tế đã chạy** là: **M0 (T-030) DONE → SIM-1..5 → SIM-XANH P0-P5 → Track UI (U0-U4 + UX-CARDS + R1/R4) → AUDIT (A1-A3) → Cycle P/R/V/W**; **M1–M4 (T-031..T-037/T-019/T-020/T-026) hiện PAUSED**, chưa resume. Phasing/trạng thái hiện hành xem `specs/simulation/00-sim-overhaul-master.md`. Run window M1 target `[00:00,24:00)` mô tả ở mục 2 dưới đây **CHƯA được thực thi** — config pilot hiện hành vẫn chạy compatibility profile 05:00–24:00.

Thứ tự bắt buộc (roadmap gốc — xem đính chính trên về trạng thái thực chạy):
...
1. CrewAI: dùng thật hay chỉ là ý tưởng tham khảo của stakeholder? Cần đánh giá so với flow tự viết (T-005). **⚠ 2026-07-29: đã trả lời — flow tự viết (C6, 9 solver) đã build; CrewAI chỉ còn tham chiếu UX trace, xem §1.**
2. Nguồn số liệu nào đủ tin cậy để hiệu chỉnh mock demand proxy sát thực tế hơn? Research đợt 1+2 đã hoàn tất (T-001, T-012); hiện vẫn thiếu dữ liệu GSM theo giờ/khu vực nên spec dùng FACT + PROXY + MOCK assumptions có version. **⚠ Cập nhật 2026-07-29:** research đã tới **đợt 4** — đợt 3 (UPDATE-031) đảo ngược giả định policy (bỏ phạt ≤70%, thêm khoán tuần); đợt 4 (UPDATE-043) đảo 3 giả định khác (app CÓ heatmap + "Nhiệm vụ tiếp theo", 4 mức cảnh báo gian lận chính thức, hạn giải trình 48 giờ). Xem `planning/RESEARCH.md`.
