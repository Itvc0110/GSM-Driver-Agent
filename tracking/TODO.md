...
| ~~D-M3-04~~ | Multiday A/B so `rest_window` stops being INERT — **CONDITIONAL TRIAL** per Cuong 2026-08-03 | ✅ **DONE-CODE 2026-08-05** — ran 100 seeds × 3 days × 2 arms. Channel spoke **2,986** times (no longer inert). **Verdict = REVERT**, hitting **both** independent conditions of the pre-locked `luat_quyet_dinh`: Δ payout **−429.3đ** CI95 [−1142.3, +289.6] ⇒ **ns**, **and** **STOP-C fired** (`rest_min_total` −6.6%). Δ landed **inside** the pre-registered expectation [−1500, +500] ⇒ the measurement **succeeded**; REVERT is the branch the prereg **predicted** (world β=0). STOP-A/B/D OK | UPDATE-140; artifact `research/audit/2026-07-27-current-state/45-dm304-multiday.json`; `run_pair_multiday` + `scripts/run_dm304.py` + 16 tests; sever **12/12** |
| ~~D-M3-04-FIX~~ ✅ **DONE-CODE 2026-08-05 (UPDATE-142)** — hoãn = CAM KẾT + nhánh rơi ≠ WAIT; A/B chẩn đoán 30 seed đạt cả ba acceptance (rest −244→+10,9 ns). Kênh vẫn TẮT+MỀM (Q-16a). ~~🔴 `rest_window` **still harms health** after the REVERT~~ — REVERT only dropped the money claim and the adherence denominator, it did **not** change behaviour. Every CI excludes zero: `rest_min_total` **−257.9′** · `work_span_p90` **+42.3′** · `drive_min_p90` **+19.6′** | TODO (sev **CAO** — touches the health boundary §1.2b). 🔒 Keep the channel **OFF** until both land: (1) deferral must be a **COMMITMENT** (record "rest at hour X", force it at X, return the right to rest the moment the window passes); (2) the fall-through **must not be `WAIT`**. ⚠ The third condition I first proposed — *"put battery cost into the deferral decision"* — is **RETRACTED**: it rested on `charge_min` +80.5′ at **3 seeds**, which is **−25.1 ns and sign-flipped** at 30 | `specs/simulation/d-m3-04-root-cause-delta-am.md` §4; mechanism proven at both levels — code (`world.py:970`, `action := WAIT`) and 30-seed paired CIs (rest −244′ → **idle** +209.5′ = **86%**, while orders/occupied are **ns**) |
| ~~D-M3-04-FIX-PRE~~ | Distinguishing test that must run **before** building `D-M3-04-FIX`: arm B″ = arm B minus the `action := WAIT` line | ✅ **DONE 2026-08-05 (UPDATE-141) — VERDICT 1**: B″ ≡ A bit-identical 30/30 seed. If `idle_min` **and** `rest_min` both return to ~0 ⇒ line `:970` is the whole mechanism. 🔒 **≥30 seeds + paired CIs** — small n has misled three times this session | `d-m3-04-root-cause-delta-am.md` §5; ~25 min |
| ~~D-M3-06~~ | Dead `GO_SWAP`/`GO_CHARGE` branches in the rest-deferral condition — measured **0/41** | ✅ **DONE-CODE 2026-08-05 (UPDATE-142, gộp vào FIX)** — điều kiện hoãn chỉ còn REST, 2 lớp test chặn tái sinh | UPDATE-140; `D-R12` family, inverted form: code looks like it governs three mechanisms, actually governs one |
...
| **CKPT-PORTFOLIO-RESEARCH** | **RESEARCH-DONE / OWNER-DECISIONS-PENDING** — 35 raw-signal group, 33 capability và 3 storyline thật đã được map; đề xuất brief/passive/composite/recap để hướng tới 5–10 touchpoint mà không tăng popup. Chưa duyệt producer, taxonomy, threshold hoặc cadence mới | UPDATE-148 + `research/audit/2026-08-05-checkpoint-scenario-discovery/` | owner chọn P0 slice, định nghĩa touchpoint/budget, sau đó 30-seed + human usefulness review |
| **CKPT-UI-IDEA-CARDS** | **RESEARCH-DONE / IMPLEMENTATION-NOT-STARTED** — 33 UI Idea Cards, 16 composite patterns, 8 product clusters, 6 storylines và top selections đã cụ thể hóa từ UPDATE-148; chưa thay runtime/UI | UPDATE-149 + `research/audit/2026-08-05-checkpoint-ui-experience-ideas/` | cycle sau chọn một vertical slice (ưu tiên plan-change hoặc journey recap), rồi mới design/plan/implement |
...
| SOFT-ADVICE-01 | **Soft advice must not be measured**: no adherence denominator, no `followed`, only a neutral Hide button (Cuong 2026-08-03). Third tier of the health↔money exchange rate that spec §1.2b blocked at the objective and world tiers — this one lives in the PRODUCT | DONE-CODE / WAITING-VERDICT (V-26) | UPDATE-135; `tracking/QUYET-DINH-2026-08-03-khuyen-mem-khong-do.md`; registry `advice_topics.py` + fail-closed gate (**63 tests** across 4 new files (measured 2026-08-03: 15 registry + 29 soft-advice + 8 OSRM + 11 env-loader — recount with `pytest --collect-only`, do NOT trust this number blind); sever-restore 4/4 then 6/6 fired). **Nothing running changes today** — no soft card exists yet; this is rails + a gate so the first one cannot silently land in the measured table |
| SOFT-ADVICE-02 | Weather card using the `soft` render mode (topic `"weather"`), and Flutter (`ui/driver_app/`) honouring the same boundary | TODO — **Khanh** | UPDATE-135; backend/web rails ready; Flutter not checked (Khanh's active claim) |
| D-ENV-01 | `OSRM_BASE_URL` was documented as tier 1 of driver-app routing but **no runtime code read it**; the second OSRM mirror had a **typo'd hostname** (`router.project.osrm.org` — TLS hostname mismatch) so it never worked; `GRAPHHOPPER_API_KEY` was absent from Cuong's `.env` (gitignored, so Khanh's edit never propagated) making tier 2 fail silently | FIXED | UPDATE-135; found by **calling every key for real**, not by reading code. Gate `ui/backend/tests/test_osrm_endpoints_wired.py` (8 tests; sever-restore with REAL pytest 8/8 fired — the earlier "2/2" scripts never ran pytest, see UPDATE-135 §5c). ⚠ Both live OSRM hosts resolve to the SAME IP (5.148.170.168) — swapping mirrors gives **no** extra rate-limit headroom |
| SOFT-ADVICE-03 | **Same boundary, FOURTH write path**: AdviceCheckpoint v2 sat entirely outside it — own store (`CheckpointStore` ≠ `AdviceEventLog`), own `topic` vocabulary **intersecting the registry in ZERO elements**, and `rest` genuinely produced (`checkpoint.py:134`, S7) ⇒ a `rest` checkpoint accepted `response: accepted`, i.e. **a consent trace for a REST recommendation was being written**. No number was wrong yet (`adherence_view` never sees the v2 store) but the data **accumulates** | DONE-CODE / WAITING-VERDICT (V-27) | UPDATE-137; QĐ-4 = option **(b) unify**. Unifies **authority, not names** — `advice_v2.json` strings and stored records unchanged. `record_response` raises `CheckpointSoftAdviceError` → **422** (own class, not 409: 409 implies *retry may work*, this is permanently forbidden); `dismissed`/`expanded` still accepted. 4 gates incl. a **producer** gate anchored on `_topic_for_action`, because the AST scanner is blind to `return "x"` — which is exactly where v2 topics are born. sever-restore **8/8** |
| D-QD4-03 | **The health boundary has TWO directions and only one was guarded.** The *extend-shift* channel was measured for adherence with **zero** health rails, while *defer-rest* had three — even though `policy_locks.py:40-42` files both levers in the **same family** (*"kéo dài thời gian làm việc vì tiền"*). It *did* read `online_min`, but as a **productivity denominator**, not a fatigue gate ⇒ a tired-but-productive driver was still told to extend | DONE-CODE (UPDATE-138) | Cuong chose **(b)** keep-measured + add rails, not soft-classify: the advice ("40 points from the next tier") genuinely IS economic — the fault was advising without a rail, not measuring. Rails `soc_low`/`fatigued`/**`would_exceed_fatigue`** + observation path into tier-5 guardrail. Measured: `would_exceed_fatigue` is **~85%** of blocks ⇒ the current-state-only variant would have caught ~11%. Behaviour-neutral on the default config **measured** (per-actor fingerprint 5/5 seeds identical) — measured *before* claiming it this time, unlike V-29. Sever 11/11; the *"drop the veto log"* mutation **initially SLIPPED**, exposing a gate that checked both ends but not the middle |
| D-QD4-05 | `online_min` is a **PROXY** for fatigue — it **includes rest time** (`D-M3-19`). Both rest and extend rails now gate on it | TODO (sev TB) | UPDATE-138; split `drive_min` out for **both** channels at once — fixing one side creates the very asymmetry `D-QD4-03` just removed |
| ~~D-QD4-01~~ | `rest` in v2 conflates `rest_window` (deferring rest = `C2′`, economic, **measured**) with `rest_nudge` (soft) | ✅ **SELF-CANCELLED 2026-08-05** — exactly the condition written into the row itself (*"self-cancels if D-M3-04 REVERTs"*). `D-M3-04` **did** REVERT, so `rest`/`rest_nudge`/`rest_window` now sit in the **same soft class**: both branches of `checkpoint.py:134` resolve to `soft`, fail-closed still holds, and there is no denominator left to lose | UPDATE-140. 🔓 **Reopen if** `D-M3-04-FIX` ships and the channel returns as an **economic** advice — the ambiguity regains consequences the moment `rest_window` goes back to `MEASURED` |
| D-QD4-02 | QĐ-4 step 3 — bring `CheckpointStore` into the shared measurement path | DEFERRED (sev THAP) | UPDATE-137; **not needed for the boundary** (post-fix data is clean at source); it is a MEASUREMENT feature dragging in the two-path join (`adherence-measurement.md` §(c)#2). ⚠ Before trusting that store: Cuong's machine has **no** `advice_checkpoint.db` (checked 2026-08-04) but **Khanh's was not checked** — pre-fix records may carry `accepted` on soft topics |
| D-ENV-02 | `GOOGLE_MAPS_API_KEY` is dead: returns `REQUEST_DENIED "API key is invalid"`, wrong format (64 hex, not `AIza…`), and **no code reads it** | LABELLED (not deleted) | UPDATE-135; labelled in `.env` + `.env.example` so nobody assumes Google Maps works. Decide later whether to obtain a real key or drop the variable |

| CYCLE-3 | **Khoá config MA + default im lặng** — `Config.get(dotted, default)` trả default im lặng khi path vắng; 3 dòng sống nhờ đó (`advice_bridge.py:202` đọc `orders.trip_km_median` trong khi yaml có `demand.trip_km_median`; `:209` tương tự; `world.py:172` đọc `dispatcher.offer_cooldown_min` **không có trong yaml**). Vô hại tới nay vì default TÌNH CỜ TRÙNG ⇒ ai sweep khoá thật sẽ thấy Δ=0 và kết luận SAI. ⭐ Cổng cũ khớp theo **TÊN LÁ** nên **chính dòng hỏng bảo lãnh cho khoá đúng** — cổng bị con bug nó đi bắt qua mặt | DONE-CODE | UPDATE-180. Cổng mới `tests/test_config_key_ton_tai.py` (chiều NGƯỢC code→config, AST, khớp đường dẫn ĐẦY ĐỦ) + `test_config_flags_wired._co_nguoi_doc` sửa sang khớp đường dẫn. Khoá ma **4→0**; **BIT-IDENTICAL 5/5 seed** (đo bằng `git stash`, không lập luận). ⚠ Bản đầu ghim 0 call-site `cfg.get(<biến>)` — chạy ra **5 ca chính đáng** ⇒ đổi sang ghim DANH MỤC, nếu không cổng luôn đỏ và sẽ bị tắt |
| CYCLE-4 | **Cổng một chiều tầng 5 KHÔNG soi 7 khoá nó khai là có soi.** `_ONE_WAY_PREFIXES` đẩy `veto_`/`xveto_`/`commit_` khỏi bảng significance hai chiều, nhưng `aggregate_health_guardrail` chép tay chỉ `veto_*` và `health_guardrail_flags` chỉ lặp `REST_RAILS` ⇒ **7 khoá ngoài tầm soi, 9 khoá vắng `a_mean`**, trong khi artifact in `"one_way_gate": health_guardrail_flags` cạnh chúng = **lời khai quản trị không có thật**. Lần **thứ BA** cùng khuôn: bản vá trước nối chiều ĐI RA, quên chiều ĐI VÀO | DONE-CODE | UPDATE-180. Danh sách khoá nay **SUY RA từ hằng rail**; cổng soi cả `EXTEND_RAILS` + sổ cam kết + nhánh MỚI *"rail CHƯA TỪNG BẮN dù cổng có chạy"* (cổng cũ mù vì đòi `va≥20`). `RAIL_KHAI_TRO` bắt khai **lý do + ĐIỀU KIỆN MỞ LẠI**. Vùng mù **7→0**, vắng `a_mean` **9→0**. ⚠ **0 thay đổi số hôm nay** (2 kênh liên quan đang TẮT) — giá trị là ĐIỀU KIỆN TIÊN QUYẾT cho mọi phép đo kênh ngủ |
| CYCLE-2 | **Năm bề mặt bằng chứng đã bị rút / sai trạng thái vẫn đang biện minh cho code**, và lan truyền **đo được 2 lần** | DONE-CODE (docs) | UPDATE-180. E1 báo cáo mentor: thêm **cột Trạng thái**, tự đếm `.solve()` ra **5/9 solver có 0 lời gọi** (bản đồ ghi 3 — tôi sửa theo phép đếm của mình) · E2 khẳng định *"vách đá mất 34k"* đã bị RÚT ở UPDATE-048 (30 seed: **+18.207đ DƯƠNG**) nhưng còn nguyên ở config **và** bản chép trong `advice_bridge` · E3 `ACTIONS` là mã hoá chỉ số **không phải** tie-break (thật: ONLINE→SWAP→REST→END) · E4 *"consumer đủ số tính net"* SAI dưới config ship · E5 hai nhãn `OBSERVED-CONFIG` phải là `ASSUMED-DEFAULT` (⚠ **giá trị** vẫn đúng nên kết luận ~40% VẪN ĐỨNG — sai là **provenance**) |
| NO-OP-SWEEP | ⛔ **Chặn hai phép đo vô nghĩa đã nằm sẵn trong hồ sơ**: sweep `cash_cost ∈ {0,70,150,250}` là **NO-OP** — `cash_km` chỉ nhân vào nhánh ONLINE nên là phép co giãn đơn điệu, argmax không đổi cho tới khi `online_net` đổi dấu; **bất biến TỪNG BIT trên [0; 4.325]đ/km**, ngưỡng lật = **17–62× giá thật**. Δ=0 rất dễ đọc thành *"chi phí không quan trọng"* | FIXED (docs) | UPDATE-180; sửa điều kiện mở lại `D-E4-01` + gạch bỏ ở `PLAN-cycle-wx-2026-07-29.md:108`. Chỉ **`swap_fee`** mới là phanh thật |
