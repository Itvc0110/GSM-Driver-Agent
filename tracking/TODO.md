...
| **D-M3-04-FIX** | 🔴 `rest_window` **still harms health** after the REVERT — REVERT only dropped the money claim and the adherence denominator, it did **not** change behaviour. Every CI excludes zero: `rest_min_total` **−257.9′** · `work_span_p90` **+42.3′** · `drive_min_p90` **+19.6′** | TODO (sev **CAO** — touches the health boundary §1.2b). 🔒 Keep the channel **OFF** until both land: (1) deferral must be a **COMMITMENT** (record "rest at hour X", force it at X, return the right to rest the moment the window passes); (2) the fall-through **must not be `WAIT`**. ⚠ The third condition I first proposed — *"put battery cost into the deferral decision"* — is **RETRACTED**: it rested on `charge_min` +80.5′ at **3 seeds**, which is **−25.1 ns and sign-flipped** at 30 | `specs/simulation/d-m3-04-root-cause-delta-am.md` §4; mechanism proven at both levels — code (`world.py:970`, `action := WAIT`) and 30-seed paired CIs (rest −244′ → **idle** +209.5′ = **86%**, while orders/occupied are **ns**) |
| **D-M3-04-FIX-PRE** | Distinguishing test that must run **before** building `D-M3-04-FIX`: arm B″ = arm B minus the `action := WAIT` line | TODO (sev TRUNG — **blocks** `D-M3-04-FIX`). If `idle_min` **and** `rest_min` both return to ~0 ⇒ line `:970` is the whole mechanism. 🔒 **≥30 seeds + paired CIs** — small n has misled three times this session | `d-m3-04-root-cause-delta-am.md` §5; ~25 min |
| **D-M3-06** | Dead `GO_SWAP`/`GO_CHARGE` branches in the rest-deferral condition (`world.py:959`) — measured **0/41** deferrals came from swap/charge; `soc_low` blocks them first | TODO (sev THAP — produces no wrong number, only misstates the mechanism's scope) | UPDATE-140; `D-R12` family, inverted form: code looks like it governs three mechanisms, actually governs one |
