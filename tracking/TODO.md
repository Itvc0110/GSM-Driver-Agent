...
| T-030 | M0: preserve/audit working diff Stage A–C + baseline manifest + canonical lifecycle/conservation/determinism invariants | READY | — | Việc tiếp theo; phải self-claim. Reproduce/classify từng flaw P0, failing regression trước narrow fix; không stage/commit nguyên khối working diff. Gate diagnostic visualization tối thiểu. |
| T-031 | M1: full `[00:00,24:00)` dynamic daily actor pool + NHPP/piecewise-linear demand + bin validation | BLOCKED | — | Phụ thuộc T-030. `actors.n` = daily pool; participation/shift/split-shift/fatigue/offline; demand per-event và bins 1/5/15 phút; calibration mặc định ≥30 seeds. |
