...
| T-009 | UI/UX tạm: clone <https://rag-xanh-sm-v1.vercel.app/> bằng [ai-website-cloner-template](https://github.com/JCodesMore/ai-website-cloner-template), mobile-first | TODO | — | Chỉ mở sau khi flow v2 được visual approve + checkpoint merge; cần vòng thông báo/hỏi/plan riêng |
...
| T-018 | Build simulator core PILOT: world + actors + dispatcher + twin-runner CRN | READY (audit-hardened) | — | DoD-core: determinism, sensitivity, calibration B-arm. Specs: `simulation-pilot-world` (DoD tách 2 tầng) + `sim-policy-bundle-v0` (fare/share/mốc NGÀY/chi phí track) + `advisor-optimization-layer-a` §1 (behavior model B) + §4 (harmonize: run window 05–24h, OD buffer ring, UNSEEN, res 9 weights) |
