...
Cập nhật: 2026-07-20. Trạng thái: `TODO` / `READY` / `DOING` / `VALIDATING` / `DONE` / `BLOCKED`. Owner theo cơ chế **tự nhận việc (self-claim)** đầu session — không ai giao việc (xem `ASSIGNMENTS.md`). Khi làm xong một mục phải có UPDATE tương ứng trong `tracking/updates/`.
