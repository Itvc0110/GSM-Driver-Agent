...
DEFAULT_PARAMS = {
    "p_accept": 0.9,               # xác suất nhận đơn (bản đầu; cá nhân hóa sau)
    "avg_dist_km": 3.0,            # quãng đường trung bình historical (→ payout/đơn)
    "soc_bands": 10,               # rời rạc hóa SOC
    "points_bands": 16,            # rời rạc hóa điểm cho terminal
    "points_band_size": 15,        # mỗi band = 15 điểm (0..240)
    "soc_cost_per_bucket": 1,      # SOC band tiêu hao/bucket online
...
def _required_rest(B: int, params: dict) -> int:
    """Số bucket nghỉ tối thiểu cho ca còn B bucket (mỗi 4h = 8 bucket → 1 nghỉ)."""
    return min(B, (B * 30 // 240) * params["rest_min_per_4h"])
...
def _forecast_arrays(spi: dict, params: dict, demand_scale: float):
    ...
    eo = [float(fc[i]["expected_orders"]) * demand_scale if i < len(fc) else 0.0 for i in range(B)]
    hrs = [_hour(fc[i]["bucket"]) if i < len(fc) else 12 for i in range(B)]
    return B, eo, hrs
...
def _solve_dp(spi: dict, policy: PolicyBundle, params: dict, demand_scale: float = 1.0):
    ...
    B, eo, hrs = _forecast_arrays(spi, params, demand_scale)
    if B <= 0:
    ...
    soc_cost = int(params["soc_cost_per_bucket"])
    ...
    for soc in range(NS):
        for pb in range(NB):
            V[B, soc, pb, 0] = policy.bonus_at(pb * PBS)
        ...
        for soc in range(NS):
            ...
            for pb in range(NB):
                ...
                for rl in range(R + 1):
                    ...
                    if rl == 0:
                        v = policy.bonus_at(pb * PBS)  # kết ca: chốt bonus hiện có
                        if v > best_v:
    ...
    for b in range(B):
        ...
        if act == "ONLINE":
            ...
            ended = True
    exp_payout += policy.bonus_at(proj_points)
...
def _baseline_naive_rest(spi: dict, policy: PolicyBundle, params: dict,
    ...
    for i in range(B):
        ...
        soc -= int(params["soc_cost_per_bucket"])
    payout += policy.bonus_at(pts)
    return payout
...
def solve(spi: dict, policy: PolicyBundle, params: dict | None = None) -> dict:
    ...
    schedule = [{"bucket": (fc[i]["bucket"] if i < len(fc) else None), "action": schedule_acts[i]}
