...
Cập nhật: 2026-08-06. Đây là file điều khiển hành vi bắt buộc cho mọi AI coding agent làm việc trong repo này. Khi có xung đột giữa file này và tài liệu khác, **file này thắng** (trừ khi Cường/Khánh nói khác trực tiếp trong hội thoại).
...
| `tracking/` | **`BOOTSTRAP-SESSION.md` = điểm vào cho session mới** (state · hàng đợi · §5 12 bẫy đã sập · **§5b la bàn quyết định**) · **`PLAN-2026-08-06-lich-trinh-cai-thien.md` = lịch trình hiện hành** (4 sóng + "Bài học đã docs" + "KHÔNG LÀM"; `PLAN-2026-07-30-hang-doi-cong-viec.md` là bản trước, còn hiệu lực cho §8) · TODO (backlog), ASSIGNMENTS (tự nhận việc — không ai giao việc), DEFERRED, `updates/` (UPDATE-###), `PENDING-REVIEW.md`, `HANDOFF-*`, `SOI-*`, `QUYET-DINH-*`, `PHAN-QUYET-*`, `OPEN-THREADS-*`, `DIRECTIVES-*`, `BACKLOG-QUESTIONS-*` |
| `tracking/PROJECT-GRAPH.md` | Bản đồ canonical của UPDATE hiện hành: route đọc theo task, dependency/correction edges, trạng thái hiện hành, pending gates và quota loop. **155 file `UPDATE-*.md`, số cao nhất UPDATE-160 (đếm bằng lệnh 2026-08-06)**; §3.7 phủ 074–093, §3.7b phủ chương trình TỐI ƯU ADVISOR E1–E5 (151–160); route đọc theo task ở `PROJECT-GRAPH-2026-07-29-addendum.md`. ⚠ **Điểm vào session mới KHÔNG phải file này mà là `tracking/BOOTSTRAP-SESSION.md`** — nó có state + hàng đợi + **§5 mười hai bẫy đã sập thật** + **§5b LA BÀN QUYẾT ĐỊNH** (quyết định loại nào thì đọc file nào) |
...
| `tests/` | **1155 test** ở `tests/` + **201** ở `ui/backend/tests/` = **1356** (đo 2026-08-06, sau merge PR #6; 2F còn lại đều của Khánh). ⚠ **`pytest -q` từ root CHỈ thu `tests/`** (`pyproject.toml` `testpaths = ["tests"]`) ⇒ **BỎ toàn bộ suite đường sản phẩm**. Muốn nói "suite xanh" thì phải chạy **CẢ HAI**: `uv run pytest -q` **và** `uv run pytest -q ui/backend/tests`. Bẫy này từng làm một con số suite bị báo thiếu cả trăm test. **Đếm lại mỗi lần trích — các con số này đã stale 3 lần** |
